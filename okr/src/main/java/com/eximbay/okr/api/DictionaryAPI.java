...
public class DictionaryAPI {
    ...
    public List<DictionaryViewModel> getDictionaryData() {
        ...
        for (DictionaryViewModel model : viewModels) {
            Optional<CodeList> dictionaryType = selectTypeModel.getDictionaryType().stream()
                    .filter(d -> d.getCodeListId().getCode().equals(model.getDictionaryType())).findFirst();
            ...
            Optional<CodeList> jobType = selectTypeModel.getJobType().stream()
                    .filter(d -> d.getCodeListId().getCode().equals(model.getJobType())).findFirst();
            model.setJobTypeCodeName(jobType.get().getCodeName());

            if(model.getDictionaryType().equals(DictionaryType.COMPETENCE)){
            Optional<CodeList> categoryGroup = selectTypeModel.getCategoryGroup().stream()
                    .filter(d -> d.getCodeListId().getCode().equals(model.getCategoryGroup())).findFirst();
            ...
            Optional<CodeList> category = selectTypeModel.getCategory().stream()
                    .filter(d -> d.getCodeListId().getCode().equals(model.getCategory())).findFirst();
    ...
    public List<DictionaryViewModel> getDictionaryOfKeyResult() {
        ...
        List<Dictionary> dictionaries = dictionaryRepository.findAll(dictionaryQuery
                .findByDictionaryType(DictionaryType.KEY_RESULT).and(dictionaryQuery.findActiveDictionaryOnly()));
        ...
        for (DictionaryViewModel model : viewModels) {
            ...
            Optional<CodeList> availableTaskType = selectTypeModel.getTaskType().stream()
                    .filter(d -> d.getCodeListId().getCode().equals(model.getTaskType())).findFirst();
            ...
            Optional<CodeList> availableTaskMetric = selectTypeModel.getTaskMetric().stream()
                    .filter(d -> d.getCodeListId().getCode().equals(model.getTaskMetric())).findFirst();
            ...
            Optional<CodeList> availableTaskIndicator = selectTypeModel.getTaskIndicator().stream()
                    .filter(d -> d.getCodeListId().getCode().equals(model.getTaskIndicator())).findFirst();
